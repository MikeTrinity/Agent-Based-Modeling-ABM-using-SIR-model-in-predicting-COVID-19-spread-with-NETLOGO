...
to setup-susceptible
  create-turtles population [setxy random-xcor random-ycor set color blue set shape "person" set size 1 set infected? false set immune? false]
end

to setup-infected
  ask n-of infected turtles [
   set color red
   set infected? true
  ]
end
...
to go
  ...
  or (count turtles with [infected?] = population)
  [stop]
  ...
  if movement-inhibitors = "Partial Lockdown" [ask turtles[move2]]
  if movement-inhibitors = "Full Lockdown" [ask turtles[move3]]
  tick
end
...
to infect-susceptibles ;; S -> I
  ask turtles [
    ...
    if (random-float 1 <  1 - (((1 - infection-rate) ^ infected-neighbors)) and not immune?)
    [set infected? true]
  ]
end
...
"susceptible" 1.0 0 -14730904 true "" "plot (count turtles with [not infected? and not immune?]) / population"
...
movement-inhibitors
movement-inhibitors
"None" "Partial Lockdown" "Full Lockdown"
2
